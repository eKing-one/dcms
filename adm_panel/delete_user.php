...
user_access('user_delete', null, 'index.php?' . session_id());
adm_check();
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($chat_post_coll != 0) $chat_post = "$chat_post +$chat_post_coll*";
}
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($k_them_coll != 0) $k_them = "$k_them +$k_them_coll*";
}
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($k_p_forum_coll != 0) $k_p_forum = "$k_p_forum +$k_p_forum_coll*";
}
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($zakl_coll != 0) $zakl = "$zakl +$zakl_coll*";
}
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($guest_coll != 0) $guest = "$guest +$guest_coll*";
}
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($konts_coll != 0) $konts = "$konts +$konts_coll*";
}
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($mail_coll != 0) $mail = "$mail +$mail_coll*";
}
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($news_komm_coll != 0) $news_komm = "$news_komm +$news_komm_coll*";
}
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($user_voice_coll != 0) $user_voice = "$user_voice +$user_voice_coll*";
}
...
if (count($collisions) > 1 && isset($_GET['all'])) {
	...
	if ($downnik_coll != 0) $downnik = "$downnik +$downnik_coll*";
}
