...
user_access('adm_themes', null, 'index.php?' . session_id());
adm_check();
...
if (isset($_FILES['file']) && filesize($_FILES['file']['tmp_name']) != 0) {
	...
	if (!isset($err)) {
		...
		if ($name != NULL) delete_dir(PCLZIP_OPT_PATH, H . 'style/themes/' . $name);
		$zip->extract(PCLZIP_OPT_PATH, H.'style/themes/'.$name, PCLZIP_OPT_SET_CHMOD, 0777,PCLZIP_OPT_BY_PREG, "#^[^\.ht]+#ui");
		if (isset($_POST['add_of_default']) && $_POST['add_of_default'] == 1) $them_default->extract(PCLZIP_OPT_PATH, H . 'style/themes/' . $name, PCLZIP_OPT_SET_CHMOD, 0777);
	...
	if (!isset($err)) msg('主题 "' . $name . ' (' . $them_name . ')" 成功安装');
}
err();
aut();
