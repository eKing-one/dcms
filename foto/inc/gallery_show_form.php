...
if ($user['level'] > $ank['level'] || $user['id'] == $ank['id'])
{
	if (isset($_GET['edit']) && $_GET['edit'] == 'rename')
	{
		...
		<div class="nav1">
		可以观看：<br />
		<input name="privat" type="radio" <?=($gallery['privat'] == 0 ? ' checked="checked"' : null)?> value="0" />全部 
		...
		<div class="nav2">
		可以评论：<br />
		<input name="privat_komm" type="radio" <?=($gallery['privat_komm'] == 0 ? ' checked="checked"' : null)?> value="0" />全部 
...
if ((user_access('foto_alb_del') || isset($user) && $user['id'] == $ank['id']) && isset($_GET['act']) && $_GET['act'] == 'delete')
{
	...
	<center>
	<a href="?act=delete&amp;ok"><img src="/style/icons/ok.gif" alt="*"> 移走</a> 
...
if (isset($user) && $user['id'] == $ank['id'] && isset($_GET['act']) && $_GET['act'] == 'upload')
{
	...
	<div class="nav1">
	<b>网站上发布的照片不得：</b><br />
	* 违反现行法律、荣誉和尊严、权利和受法律保护的第三方利益，助长宗教、种族或种族不和，包含暴力或不人道对待动物的场面等;<br />
	* 淫秽或冒犯性;<br />
	* 包含麻醉药品广告;<br />
	* 侵犯未成年人的权利;<br />
	* 侵犯第三方的版权和相关权利;<br />
