...
echo "<a href='/user/info/settings.php' class='activ'>通用</a>";
echo "</div>";
...
echo "</div>";
echo "<div class='webmenu last'>";
echo "<a href='/user/info/settings.privacy.php' >隐私设置</a>";
echo "</div>";
echo "<div class='webmenu last'>";
echo "<a href='/user/info/secure.php' >密码设置</a>";
echo "</div>";
echo "</div>";
echo "<form method='post' action='?$passgen'>";
echo "聊天室中自动刷新(秒):<br /><input type='text' name='set_time_chat' value='$set[time_chat]' maxlength='3' /><br />";
echo "每页显示信息条数:<br /><input type='text' name='set_p_str' value='$set[p_str]' maxlength='3' /><br />";
...
echo "</select><br />";
echo "论坛上传文件:<br /><select name='set_files'>";
...
echo "</select><br />";
echo "自动添加联系人:<br /><select name='add_konts'>";
echo "<option value='2'" . ($user['add_konts'] == 2 ? " selected='selected'" : null) . ">阅读信息后</option>";
echo "<option value='1'" . ($user['add_konts'] == 1 ? " selected='selected'" : null) . ">互发消息后</option>";
echo "<option value='0'" . ($user['add_konts'] == 0 ? " selected='selected'" : null) . ">手动添加</option>";
echo "</select><br />";
...
echo "</select><br />";
echo "显示色情图像忽略警告:<br />";
echo "<input name='metka'" . ($user['abuld'] == 0 ? " checked='checked'" : null) . "  type='radio' value='0' />开启 ";
echo "<input name='metka'" . ($user['abuld'] == 1 ? " checked='checked'" : null) . "  type='radio' value='1' />关闭<br />";
echo "<input type='submit' name='save' value='保存设置' />";
echo "</form>";
echo "<div class=\"foot\">";
echo "<img src='/style/icons/str2.gif' alt='*'> " . user::nick($user['id']) . " | ";
echo '<b>通用</b>';
echo "</div>";
