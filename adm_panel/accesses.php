...
user_access('adm_accesses', null, 'index.php?' . session_id());
adm_check();
if (isset($_GET['id_group']) && dbresult(dbquery("SELECT COUNT(*) FROM `user_group` WHERE `id` = '" . intval($_GET['id_group']) . "'"), 0)) {
	...
	if (isset($_POST['accesses'])) {
		...
		while ($post = dbassoc($q)) {
			$type = $post['type'];
			if (isset($_POST[$type]) && $_POST[$type] == 1) dbquery("INSERT INTO `user_group_access` (`id_group`, `id_access`) VALUES ('$group[id]', '$post[type]')");
		}
		msg('权限更改成功');
	}
	aut();
