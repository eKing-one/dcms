...
if (isset($_GET['act']) && isset($_GET['ok']) && $_GET['act'] == 'set' && isset($_POST['name'])) {
	...
	if (!isset($err)){
		...
		admin_log('论坛','部分',"重命名部分 {$razd['name']} 为 {$name}");
...
// 移动论坛部分
$razd=dbassoc(dbquery("SELECT * FROM `forum_r` WHERE `id` = '".intval($_GET['id_razdel'])."' AND `id_forum` = '".intval($_GET['id_forum'])."' LIMIT 1"));
if (isset($_GET['act']) && isset($_GET['ok']) && $_GET['act']=='mesto' && isset($_POST['forum']) && is_numeric($_POST['forum']) && dbresult(dbquery("SELECT COUNT(*) FROM `forum_f` WHERE `id` = '".intval($_POST['forum'])."'"),0)==1) {
	...
	admin_log('论坛','部分',"转移分区 {$razd['name']} 从子论坛 {$forum_old['name']} 到 {$forum['name']}");
	msg('该部分已成功迁移');
}
