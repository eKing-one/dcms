...
aut();
echo '<div class="foot">';
echo '<img src="/style/icons/str2.gif" alt="*"> ' . user::nick($ank['id'], 1, 0, 0) . ' | <a href="/foto/' . $ank['id'] . '/">专辑</a> | ';
...
if ($user['id'] != $ank['id'] && $gallery['pass'] != NULL) {
	...
	if (!isset($_SESSION['pass']) || $_SESSION['pass'] != $gallery['pass']) {
		...
		echo '<div class="foot">';
		echo '<img src="/style/icons/str2.gif" alt="*"> ' . user::nick($ank['id'], 1, 0, 0) . ' | <a href="/foto/' . $ank['id'] . '/">专辑</a> | <b>' . text($gallery['name']) . '</b>';
		echo '</div>';
