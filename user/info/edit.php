...
if (isset($_GET['set'])) {
	...
	if (isset($_POST['save']) && isset($_GET['set'])) {
		//----------昵称------------//
		if (isset($_GET['set']) && $_GET['set']=='nick') {
			if (dbresult(dbquery("SELECT COUNT(*) FROM `user` WHERE `nick` = '".my_esc($_POST['nick'])."'"),0)==0) {
				...
				$err[]='用户名 "'.stripcslashes(htmlspecialchars($_POST['nick'])).'" 已存在';
			}
			if (isset($_POST['nick']) && !isset($err)) {
				$user['nick'] = $_POST['nick'];
				dbquery("UPDATE `user` SET `nick` = '".my_esc($user['nick'])."' WHERE `id` = '$user[id]' LIMIT 1");
			}
		}
		...
		if (isset($_GET['set']) && $_GET['set']=='osebe') {
			if (isset($_POST['ank_o_sebe']) && strlen2($_POST['ank_o_sebe'])<=512) {
				if (preg_match('#[^\u4e00-\u9fa5\p{P}A-z0-9 _\-\=\+\(\)\*\!\?\.,]#ui',$_POST['ank_o_sebe'])) {
					$err[]='禁止字符用于”关于我”字段';
				} else {
	...
	if (isset($_GET['set']) && $_GET['set']=='osebe') echo "关于我:<br /><input type='text' name='ank_o_sebe' value='$user[ank_o_sebe]' maxlength='512' /><br />";
