...
// 获取搜索地点
if (isset($_GET['in']) && $_GET['in'] != null && $_GET['in'] != 'all' && preg_match('#^(r|f)([0-9]+)$#', $_GET['in'], $in)) {
...
if (!isset($_GET['search']) || strlen2($_GET['search']) < 2) {
	$err[] = '错误查询';
} else {
	// 对每个分割的词汇进行处理
	$s_arr = preg_split("/[\s,]+/", $_GET['search']);;
...
if (isset($s_arr_mysql)) {
	...
	$searched['sql_query'] = "SELECT 
		                      ...
		                      (isset($searched['in']['m']) && $searched['in']['m'] == 'f' ? "`forum_t`.`id_forum` = '" . $searched['in']['id'] . "' AND " : null) . // 仅在所选论坛中
		                      (isset($searched['in']['m']) && $searched['in']['m'] == 'r' ? "`forum_t`.`id_razdel` = '" . $searched['in']['id'] . "' AND " : null) . // 仅在所选部分
