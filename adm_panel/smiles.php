...
if(isset($_GET['id'])) {
	...
	if (dbresult(dbquery("SELECT COUNT(*) FROM `smile_dir` WHERE `id` = '" . intval($_GET['id']) . "'"),0) == 0) header("Location: admin.php");
...
if (isset($_GET['id'])) {
	...
	*/
	...
	if ($k_post == 0) {
		?><div class="mess">表情符号列表为空</div><?php
	}
