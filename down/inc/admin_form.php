<?php
if (user_access('down_dir_edit') && isset($_GET['act']) && $_GET['act']=='set') {
	...
	echo "<label><input type=\"checkbox\"$check name=\"upload\" value=\"1\" /> 允许上传</label><br />";
	echo "扩展名通过 \";\"分隔:<br />";
	echo "<input type='text' name='ras' value='$dir_id[ras]' /><br />";
	echo "最大文件大小：<br />";
	if ($dir_id['maxfilesize']<1024)$size=$dir_id['maxfilesize']; elseif($dir_id['maxfilesize']>=1024 && $dir_id['maxfilesize']<1048576)$size=intval($dir_id['maxfilesize']/1024); elseif($dir_id['maxfilesize']>=1048576)$size=intval($dir_id['maxfilesize']/1048576);
...
if (user_access('down_dir_create') && isset($_GET['act']) && $_GET['act']=='mkdir') {
	...
	echo '<label><input type="checkbox" name="upload" value="1" /> 允许上传</label><br />';
	echo '扩展名通过 \";\"分隔:<br />';
	...
	echo '</select><br />';
	echo '*服务器设置不允许上传超过: '.size_file($upload_max_filesize).'<br />';
	echo '<img src="/style/icons/ok.gif" alt="*"> <input class="submit" type="submit" value="创建文件夹" /> ';
	echo '[<img src="/style/icons/delete.gif" alt="*"> <a href="?">取消</a>]<br />';
	echo '</form>';
}
if (user_access('down_dir_edit') && isset($_GET['act']) && $_GET['act']=='rename' && $l!='/') {
	...
	echo '<img src="/style/icons/ok.gif" alt="*"> <input class="submit" type="submit" value="改名" /> ';
	echo '[<img src="/style/icons/delete.gif" alt="*"> <a href="?">取消</a>]<br />';
	echo '</form>';
}
if (user_access('down_dir_edit') && isset($_GET['act']) && $_GET['act']=='mesto' && $l!='/') {
	...
	echo '<option value="/">[底部]</option>';
	$q=dbquery("SELECT DISTINCT `dir` FROM `downnik_dir` WHERE `dir` not like '$l%' ORDER BY 'dir' ASC");
	while ($post = dbassoc($q)) {
		echo '<option value="'.$post['dir'].'">'.$post['dir'].'</option>';
	}
	echo '</select><br />';
	echo '<img src="/style/icons/ok.gif" alt="*"> <input class="submit" type="submit" value="移动" />';
	echo '[<img src="/style/icons/delete.gif" alt="*"> <a href="?">取消</a>]<br />';
	echo '</form>';
}
...
if (user_access('down_dir_edit') || user_access('down_dir_delete') || user_access('down_dir_create')) {
	...
	if ($l!='/') {
		...
		if (user_access('down_dir_delete') && $dir_id['my'] == 0)
