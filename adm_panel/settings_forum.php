...
user_access('adm_set_forum', null, 'index.php?' . session_id());
adm_check();
$set['title'] = '论坛设置';
...
if ($temp_set['show_num_post'] == 1) {
	$sel = ' selected="selected"';
} else {
	$sel = NULL;
}
echo "<option value=\"1\"{$sel}>显示</option>";
if ($temp_set['show_num_post'] == 0) {
	$sel = ' selected="selected"';
} else {
	$sel = NULL;
}
...
if ($temp_set['forum_counter'] == 1) {
	$sel = ' selected="selected"';
} else {
	$sel = NULL;
}
echo "<option value=\"1\"{$sel}>人数</option>";
if ($temp_set['forum_counter'] == 0) {
	$sel = ' selected="selected"';
} else {
	$sel = NULL;
}
...
if ($temp_set['echo_rassh_forum'] == 1) {
	$sel = ' selected="selected"';
} else {
	$sel = NULL;
}
echo "<option value=\"1\"{$sel}>显示</option>";
if ($temp_set['echo_rassh_forum'] == 0) {
	$sel = ' selected="selected"';
} else {
	$sel = NULL;
}
...
if (user_access('adm_panel_show')) {
	echo "<div class='foot'>";
	if (user_access('adm_forum_sinc')) echo "&raquo;<a href='/adm_panel/forum_sinc.php'>论坛表的同步</a><br />";
