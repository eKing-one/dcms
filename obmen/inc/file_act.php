...
if ((user_access('obmen_file_delete') || $user['id']==$file_id['id_user'])  && isset($_GET['act']) && $_GET['act']=='edit' && isset($_GET['ok']) && $l!='/')
{
	...
	if(!isset($err)){
		dbquery("UPDATE `obmennik_files` SET `metka` = '".$metka."', `name` = '".$name."',`opis` = '".$opis."' WHERE `id` = '$file_id[id]' LIMIT 1");
		$_SESSION['message']= '文件编辑成功';
		admin_log('交换器', '编辑文件', "编辑文件 [url=/obmen$dir_id[dir]$name.$file_id[ras]?showinfo]$file_id[name][/url]");
...
if ((user_access('obmen_file_delete') or $user['id']==$file_id['id_user']) && isset($_GET['act']) && $_GET['act']=='delete' && isset($_GET['ok']) && $l!='/')
{
	...
	unlink(H.'sys/obmen/files/'.$file_id['id'].'.dat');	$_SESSION['message']= '文件已成功删除';
