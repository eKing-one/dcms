...
user_access('adm_set_user', null, 'index.php?' . session_id());
adm_check();
$set['title'] = '用户设置';
...
if (isset($_POST['save'])) {
	...
	if ($_POST['allow_guest_rules_page'] == 1 || $_POST['allow_guest_rules_page'] == 0) {
		$temp_set['allow_guest_rules_page'] = intval($_POST['allow_guest_rules_page']);
	}	
...
if ($temp_set['allow_guest_rules_page'] == 1) {
	$sel = ' selected="selected"';
} else {
	$sel = NULL;
}
echo "<option value=\"1\"{$sel}>允许</option>";
if ($temp_set['allow_guest_rules_page'] == 0) {
	$sel = ' selected="selected"';
} else {
	$sel = NULL;
}
